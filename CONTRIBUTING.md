...
4. Test each package by itself, and together when a change can affect their
   ...
   6. open a fold of Claude's activity, then turn Focus mode off and on
      with `/focus`;
   7. on Windows, also: the pane offers `node`, the console window Logseq
...
package ships. The ZIP is created before that local file is copied, and archive
verification rejects unexpected members.
