...
- Every bullet on the rail uses a 10px dot inside Logseq's transparent 16px control, regardless of heading size or folded state. Only its position follows the first line. Parents have a crisp 2px ring with a 2px gap; expanded parents hide the central dot; there is no glow, including on hover. Fill is declared on the block's own control column; upstream's important hover fill is answered from the same solid fill variable.
