...
- A block that carries the hierarchy — one Logseq marks `haschild="true"`, or one whose first line renders or is typed as a heading — takes the colour of its own depth for its bullet: `--hc-rail-depth-1` to `--hc-rail-depth-8` in brightness order, amber at the top level, repeating below the eighth. Every other block keeps a white bullet. The colour is declared on the block's own control column, so a child's bullet never takes its parent's.
