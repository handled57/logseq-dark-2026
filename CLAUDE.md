...
- In the main editor's page tree, every rendered block hangs its own bullet on the configured rail layout. Flat is the default: one line runs behind all bullets from the centre of the first bullet to the centre of the last, and Logseq's control column is pulled left by the indentation of its nesting level (one rule per level, `29px` each) plus `--hc-rail-offset`. Connect the dots uses the same depth compensation to align all bullets in that column; it draws 2px vertical rails only between consecutive visible blocks at the same depth. Both halves of a depth change are hidden, so expanded descendants interrupt a sibling rail. No curves or horizontal arms connect nesting columns. Every visible segment begins and ends at a bullet. The runtime skips front matter, embedded blocks and hidden descendants, and keeps each content root independent. It adds no child-group padding. Both layouts hand the control column's negative margin back on its right, so the content hierarchy never moves. `--hc-rail-offset` shrinks on a narrow window and on the full-width route, where the only room left of the tree is the scroll container's `2rem` padding.
