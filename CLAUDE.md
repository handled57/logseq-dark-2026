...
- Every bullet on the rail is a solid 10px dot inside Logseq's transparent 16px control, regardless of heading size or folded state. Only its position follows the first line. Only collapsed parents have a crisp 1px ring with a 2px gap; there is no glow, including on hover. Fill is declared on the block's own control column; upstream's important hover fill is answered from the same solid fill variable.
