...
- A bullet sits `--hc-rail-bullet-y` below the top of its row, on the middle of the block's first line: `12px` for ordinary prose, `1.75em` of the heading's own size for a heading (view and editor), and further for the box a quote, passage, admonition, code block or table opens with. The fold arrow, an ordered list's number and both ends of the line are measured from that variable. Hovering a block preserves its bullet shape, size and color. The rail's arithmetic is derived from pinned upstream declarations in `test/cascade.test.mjs`.
...
- Every bullet on the rail uses a 10px dot inside Logseq's transparent 16px control, regardless of heading size or folded state. Only its position follows the first line. Collapsed parents have a crisp 2px outer ring with a 2px gap; expanded parents use a hollow 2px ring with a 10px outer diameter matching a leaf dot; an opaque black backing masks the rail inside the ring and gap. There is no glow, including on hover. Fill is declared on the block's own control column; upstream's important hover fill is answered from the same solid fill variable.
