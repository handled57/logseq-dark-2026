...
- In the main editor's page tree, every rendered block hangs its own bullet on one vertical rail, and a `--vscode-hc-cyan` line runs behind the bullets from the centre of the first to the centre of the last. The bullet is repositioned, never cloned: Logseq's control column is pulled left by the indentation of its nesting level (one rule per level, `29px` each) and handed the same distance back as margin, so the content column keeps Logseq's hierarchy. The rail's arithmetic is derived from pinned upstream declarations in `test/cascade.test.mjs`.
