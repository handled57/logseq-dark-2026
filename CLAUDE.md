...
- In the main editor's page tree, every rendered block hangs its own bullet on the configured rail layout. Flat is the default: one line runs behind all bullets from the centre of the first bullet to the end of the last block, and Logseq's control column is pulled left by the indentation of its nesting level (one rule per level, `29px` each) plus `--hc-rail-offset`. Branched takes back only `--hc-rail-offset`, leaving the bullets' 29px depth steps visible; it draws 2px vertical rails only between consecutive visible blocks at the same depth. Both halves of a depth change are hidden, so expanded descendants interrupt a sibling rail. No curves or horizontal arms connect nesting columns. The first content block keeps a short opening rail above its bullet and the last has no trailing rail. The runtime skips front matter, embedded blocks and hidden descendants, and keeps each content root independent. It adds no child-group padding. Both layouts hand the control column's negative margin back on its right, so the content hierarchy never moves. `--hc-rail-offset` shrinks on a narrow window and on the full-width route, where the only room left of the tree is the scroll container's `2rem` padding.
- A bullet sits `--hc-rail-bullet-y` below the top of its row, on the middle of the block's first line: `12px` for ordinary prose, `1.75em` of the heading's own size for a heading (view and editor), and further for the box a quote, passage, admonition, code block or table opens with. The fold arrow, an ordered list's number and both ends of the line are measured from that variable. Hovering a block keeps its bullet solid at the same size and color. The rail's arithmetic is derived from pinned upstream declarations in `test/cascade.test.mjs`.
...
- A block that carries the hierarchy — one Logseq marks `haschild="true"`, or one whose first line renders or is typed as a heading — takes the colour of its own depth for its bullet: `--hc-rail-depth-1` to `--hc-rail-depth-8` in brightness order, amber at the top level, repeating below the eighth. Every other block keeps a white bullet in Flat; Branched colors every bullet by depth. The colour is declared on the block's own control column, so a child's bullet never takes its parent's.
- In Branched, every bullet (including leaves) and its rail take the block's depth colour, so both endpoints match. In Flat, the rail's line takes no depth colour. Every block paints its stretch in `--hc-rail-default-color`, the rail line and the theme's one configurable colour: it defaults to `--vscode-hc-border`, and `index.js` writes the **Rail color** setting over it inline on the host's `body` — the palette's selector list declares every variable on `:root body` too, so a value set on the root element never reaches a block. The eight depth colours the bullets carry are not the setting's to change.
- Every bullet on the rail uses a 10px dot inside Logseq's transparent 16px control, regardless of heading size or folded state. Only its position follows the first line. Parents have a crisp 2px ring with a 2px gap; expanded parents hide the central dot; an opaque black backing masks the rail inside the ring and gap. There is no glow, including on hover. Fill is declared on the block's own control column; upstream's important hover fill is answered from the same solid fill variable.
