...
- Every bullet on the rail is a solid 9px dot inside Logseq's 16px halo, regardless of heading size or folded state. Only its position follows the first line. There are no rings or shadows, including on hover. Fill is declared on the block's own control column; upstream's important hover fill is answered from the same solid fill variable.
