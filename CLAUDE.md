...
- Every bullet on the rail is a solid 7px dot inside Logseq's 16px halo, regardless of heading size or folded state. Only its position follows the first line. There are no permanent colored rings; hovering the innermost block adds one ring in its bullet's own color. The black gap separates each dot from the rail. Fill and shadows are declared on the block's own control column; upstream's important hover fill is answered from the same solid fill variable.
