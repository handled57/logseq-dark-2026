...
- Every bullet on the rail is one size — a 7px dot inside Logseq's 16px halo — whatever its block's first line is set in; only where it sits follows that line. A block with children, `haschild="true"` whether they are showing or folded, carries one ring of its own colour around the bullet, and hovering the innermost block adds one more ring outside whatever is already there; a leaf is ringed only under the pointer. A bullet is filled in its own colour except where its block stands open over its children, which empties it out to the ring alone; folding it fills it back in. `--hc-rail-bullet-gap` and `--hc-rail-bullet-ring` are the band widths, `--hc-rail-bullet-edge` is how far the rest state reaches, `--hc-rail-bullet-fill` is the inside, and the two shadow lists `--hc-rail-bullet-rings` and `--hc-rail-bullet-hover-rings` compose into the bullet's `box-shadow`. Hovering never changes a bullet's inside; upstream's important fill is answered from the same variable.
