...
- That outline stands off the text it encloses by `--hc-block-outline-pad`, 4px by default, so a hovered or selected block reads as a bordered card rather than as a line ruled through its paragraph; a selected block's fill reaches the border with it. The room is taken outside the box Logseq sizes, so nothing in the block moves or rewraps when the pointer arrives, and blocks are spaced by `--hc-block-gap` — the padding twice over, plus 4px — so two blocks selected one after the other keep their outlines apart. A graph that wants a tighter page can retune either from `custom.css`.
