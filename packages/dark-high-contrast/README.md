...
- **Branched** leaves one 29px horizontal step between nesting levels. An
  ...
  same shape at its own level. The return is a cubic curve with a vertical
  tangent at both ends, and neither connector uses a diagonal segment; a
  collapsed parent has no visible child connector.
...
3.47:1, is used only as a bullet.

Blocks that carry no hierarchy of their own — ordinary prose without children —
keep a white bullet.

The line every bullet hangs on is the **Rail color** setting, one color at
...
- Hovering a block lights its own bullet in that bullet's own color — white for ordinary prose, its own depth's color for a heading or a parent. An ancestor holding the hovered block keeps its bullet plain, the way the block highlight already behaves.
...
- That outline stands off the text it encloses by `--hc-block-outline-pad`, 6px by default, so a hovered or selected block reads as a bordered card rather than as a line ruled through its paragraph; a selected block's fill reaches the border with it. The room is taken outside the box Logseq sizes, so nothing in the block moves or rewraps when the pointer arrives, and blocks are spaced by `--hc-block-gap` — the padding twice over, plus 4px — so two blocks selected one after the other keep their outlines apart. A child block is nested inside its parent rather than laid out after it, so every nested group opens on that same space, counted so a parent and its first child stand exactly as far apart as two siblings. The orange border a block wears while it is being edited stands off on the same padding, so the border stays in one place whether a block is being read or written. Each row paints its stretch of the rail down the whole gap below it, so the line stays unbroken at any spacing. A graph that wants a tighter page can retune either variable from `custom.css`.
