...
- That outline stands off the text it encloses by `--hc-block-outline-pad`, 6px by default, so a hovered or selected block reads as a bordered card rather than as a line ruled through its paragraph; a selected block's fill reaches the border with it. The room is taken outside the box Logseq sizes, so nothing in the block moves or rewraps when the pointer arrives, and blocks are spaced by `--hc-block-gap` — the padding twice over, plus 4px — so two blocks selected one after the other keep their outlines apart. Each row paints its stretch of the rail down the whole gap below it, so the line stays unbroken at any spacing. A graph that wants a tighter page can retune either variable from `custom.css`.
