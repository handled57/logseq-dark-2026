...
- Every heading level is ruled off with a thin line in the theme's structural border. The rule starts where the heading's text starts and runs to the end of the block's content column, and it holds while the heading is being typed.
...
- Every heading level carries a thin underline in `--vscode-hc-border`, the same line the editor frame, the sidebars and the rail are drawn with. Logseq rules off only an `h1` and an `h2`, in a near-black token that cannot be seen on this theme's page, so the rule is redrawn and taken to all six levels. The line is the heading's own box: it opens where the heading's text opens and runs to the end of the block's content column, and it is drawn on the editor's textarea too, so it holds while the heading is typed. A heading quoted inside a block reference stays unruled, as upstream leaves it.
