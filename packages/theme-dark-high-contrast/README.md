...
- The PDF viewer is themed wherever it is opened, including the separate window Logseq pops it out into. That window is a plain browser popup and Logseq copies only its own stylesheet into it, so the theme hands it whichever of the main window's stylesheets it is missing — the theme, and Logseq's own icon font. A popped-out window that was already open when the theme loaded keeps Logseq's default palette until it is reopened.
...
color with a 2px gap. Expanded parents use a hollow 2px ring with a 12px
outer diameter and no outer ring.
...
Hovering a property-bearing block in the main editor reveals a cyan dot just to
the right of its bullet; keyboard focus reveals it too. Activate it with the
pointer, Enter or Space to show or hide that block's property table. This manual
choice wins over the configured hiding rules and follows the block across
rerenders and navigation for the rest of the current Logseq session; it is never
persisted and writes nothing to the graph. Page front matter, sidebars, embeds,
...
Left-clicking a block bullet expands or collapses that block; the redundant fold
arrow beside rail bullets is hidden. A block with nothing to fold stays where it is. To open a
...
- Every heading level carries a thin underline in `--vscode-hc-white`, the white the heading's own text is set in. Logseq rules off only an `h1` and an `h2`, in a near-black token that cannot be seen on this theme's page, so the rule is redrawn and taken to all six levels. The line is the heading's own box: it opens where the heading's text opens and runs to the end of the block's content column, and it is drawn on the editor's textarea too, so it holds while the heading is typed. A heading quoted inside a block reference stays unruled, as upstream leaves it.
- Every block ends on the same right edge, whether or not it is referenced anywhere. Logseq draws a block's reference count in a column beside the block's content and leaves that column zero wide on a block nothing points at, so a linked block is laid out narrower than the plain block above it and a heading's rule, a table, a box edge and the point a line wraps at all stop somewhere different down the page. The column is held open on every block instead, at `--hc-block-refs-gutter`, `2.5rem` by default — room for a three-digit count — and the counts are pushed to the far side of it so they line up with one another as well. A graph can retune the number in `custom.css`. Sidebars and dialogs keep Logseq's layout, where the room is worth more than the edge.
...
- Hovering a block leaves its surface unchanged. The raised `#1f1f1f` fill and structural outline are kept for the deliberate, persistent states: a selected block and Logseq's own block highlight.
- That persistent outline stands off the text it encloses by `--hc-block-outline-pad`, 6px by default, so a selected block reads as a bordered card rather than as a line ruled through its paragraph; its fill reaches the border with it. The room is taken outside the box Logseq sizes, and blocks are spaced by `--hc-block-gap` — the padding twice over, plus 4px — so two blocks selected one after the other keep their outlines apart. A child block is nested inside its parent rather than laid out after it, so every nested group opens on that same space, counted so a parent and its first child stand exactly as far apart as two siblings. The orange border a block wears while it is being edited stands off on the same padding. Each row paints its stretch of the rail down the whole gap below it, so the line stays unbroken at any spacing. A graph that wants a tighter page can retune either variable from `custom.css`.
