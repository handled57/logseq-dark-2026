...
- The PDF viewer is themed wherever it is opened, including the separate window Logseq pops it out into. That window is a plain browser popup and Logseq copies only its own stylesheet into it, so the theme hands it the same stylesheet the main window is wearing. A popped-out window that was already open when the theme loaded keeps Logseq's default palette until it is reopened.
